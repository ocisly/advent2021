fun main() {
    fun part1(input: List<String>): Int {
        ...
        val gamma = parsed.transposed().map { nums -> nums.groupingBy { it }.eachCount() }
    ...
    fun part2(input: List<String>): Int {
        val parsed = input.map { it.map(Char::digitToInt) }
        val o2 = findRating(parsed) { zeros, ones -> if (zeros <= ones) 1 else 0 }
        val co2 = findRating(parsed) { zeros, ones -> if (zeros <= ones) 0 else 1 }
        return o2.joinToString("").toInt(2) * co2.joinToString("").toInt(2)
    }
...
fun findRating(parsed: List<List<Int>>, pos: Int = 0, pred: (Int, Int) -> Int): List<Int> {
    ...
    val needle = parsed.transposed()[pos]
        .groupingBy { it }
        .eachCount()
        .let {
            pred(it.getValue(0), it.getValue(1))
        }
